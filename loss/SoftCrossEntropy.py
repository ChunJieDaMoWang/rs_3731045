...
class CrossEntropyLoss(nn.Module):
    ...
    def __init__(
        ...
        assert num_classes > 0, 'num_classes must greater than 0'
    ...
    def _logits_transform(self, logits: torch.Tensor):
    ...
    def _get_class_weight(self, targets: torch.Tensor):
        ...
        return class_weight.reshape(1, -1, *[1] * (targets.ndim - 1))
    ...
    def _get_loss(self, logits, targets):
        ...
        logits = self._logits_transform(logits)
        class_weight = self._get_class_weight(targets)
        targets = self._get_smooth_label(targets * mask)

        log_pt = F.log_softmax(logits, dim=1) * mask.unsqueeze(1)

        return (-class_weight * targets *
                torch.pow(1.0 - log_pt.exp() * mask.unsqueeze(1), self.gamma) *
                log_pt * mask.unsqueeze(1)).sum(dim=1)
